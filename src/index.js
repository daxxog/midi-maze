(function() {
	var _r = function(useMidi) {
		...
		// handle MidiFighter3D if we have it
		var	midiH = false;
		if(useMidi === true) {
			midiH = handleMidi();
		}
		...
		Game.initLevel = function(_Maze) {
			...
			Game.obj.ball = new Game.object(canvas, {
				...
				Game.obj.ball.add(new fabric.Circle({
				  left: Game.obj.ball.x,
				  top: Game.obj.ball.y,
				  fill: (midiH === false) ? 'black' : 'blue', // if we are using midi ball is blue
				  radius: Game.obj.ball.width / 2
				})).setPreDraw(function() {
					if(midiH === false) { // no midi, fallback to mouse
						...
						this.xAccel = midiH.x / 128;
						this.yAccel = midiH.y / 128;
						this.friction = 0.1;
					}
